...
def extract_note_title(context):
    ...
    for line in reversed(lines):
        if re.search(r'\b(?:Thuyết minh|Thuyêt minh)\s*\d+', line, re.IGNORECASE) or \
           re.match(r'^\s*(?:[IVXLCDM]+\.|\d+\.\d*)\s*[a-zA-ZÀ-ỹ]', line) or \
           any(k in line.lower() for k in ['thuyết minh', 'bảng cân đối', 'báo cáo', 'tài sản', 'nợ phải trả', 'vốn chủ']):
            return line
    if lines:
        return lines[-1]
    return "Thuyết minh chi tiết"
...
def process_single_file(file_path, output_dir, ticker_to_name):
    ...
    for t_idx, table_html in enumerate(table_blocks):
        table_num = t_idx + 1
        
        char_offset = content.find(table_html)
        if char_offset != -1:
            start_line = content[:char_offset].count('\n') + 1
            
            # Extract context: 6 lines before the table start
            pre_content = content[:char_offset].strip()
            pre_lines = pre_content.split('\n')
            context_lines = pre_lines[-6:] if len(pre_lines) >= 6 else pre_lines
